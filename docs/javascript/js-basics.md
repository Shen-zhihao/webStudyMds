...
// requestIdleCallback是在浏览器渲染后有空闲时间时执行，如果requestIdleCallback设置了第二个参数timeout，则会在超时后的下一帧强制执行回调函数
// 如果requestIdleCallback没有设置第二个参数timeout，则会在浏览器渲染后有空闲时间时执行回调函数
// 如果requestIdleCallback设置了第二个参数timeout，则会在超时后的下一帧强制执行回调函数
// 如果requestIdleCallback没有设置第二个参数timeout，则会在浏览器渲染后有空闲时间时执行回调函数
...
const asy2 = async () => {
  console.log(3);
  await setTimeout(() => {
    // 后面的任务全部进入下一个轮询，第一轮代码结束，将释放的定时器加入下一个宏任务队列
