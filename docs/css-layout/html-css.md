...
1. **用户代理样式表**（浏览器默认样式，如 `<h1>` 的默认字体大小）；
2. **作者样式表**（开发者编写的 CSS 规则）；
3. **用户样式表**（用户自定义的样式，如浏览器插件修改的样式，目前较少见）。

#### 解决层叠冲突
当多个样式规则对同一属性存在冲突时，浏览器通过以下规则确定优先级：

##### 1. 比较源的重要性
优先级顺序为： 
`!important 的作者样式 > 普通作者样式 > 用户代理样式`

##### 2. 比较选择器权重
权重由四位数表示（如 `0,1,0,1`），依次比较：

+ **行内样式**（如 `style="color: blue"`，权重 `1000`）；
+ **ID 选择器**（如 `#header`，权重 `100`）；
+ **类/伪类/属性选择器**（如 `.class`，权重 `10`）；
+ **元素/伪元素选择器**（如 `div`，权重 `1`）。

##### 3. 比较代码位置
若上述条件相同，**后声明的样式**覆盖先声明的样式。

#### 继承属性值
对于未显式声明的属性，浏览器尝试从父元素继承可继承的属性：

+ **可继承属性**：`color`、`font` 系列、`text-align`、`line-height` 等文本相关属性；
+ **不可继承属性**：`margin`、`padding`、`width`、`display` 等布局相关属性。

#### 使用默认值
若前三个步骤均未确定属性值，则使用浏览器默认值。
...
grid-template-columns: 100px repeat(3, 1fr) 10%;                  			 // 混合写法
grid-template-columns: repeat(auto-fill, minmax(100px, 1fr));     			 // auto-fill 自动填充（每行尽量多）
grid-template-columns: repeat(auto-fit, 100px);                   			 // auto-fit 自动填充（每行尽量满）
grid-template-columns: 1fr 1fr minmax(100px, 1fr);   										 // minmax() 两个参数分别为最小值和最大值。
grid-template-columns: fit-content(100px) fit-content(100px) 40px auto;  // 让尺寸适应于内容，但不超过设定的尺寸,只支持数值和百分比值
```

```css
grid-template-rows            // 使用方法同上
grid-template-rows:masonry;   // 瓦片布局，一行css实现瀑布流
```

```css
grid-auto-columns    // 控制多余的元素列排列方式，使用方法同上
```

```css
grid-auto-rows       // 控制多余的元素行排列方式，使用方法同上
```
...
```css
justify-content 指定单元格内容的水平对齐方式，属性值有：
  stretch：【默认值】拉伸，占满单元格的整个宽度。
  start：对齐单元格的起始边缘。
  end：对齐单元格的结束边缘。
  center：单元格内部居中。

align-content 指定单元格内容的垂直对齐方式，属性值有：
...
```css
grid-auto-flow : row;          				// 按照行排列（竖排）
grid-auto-flow : column;       				// 按照列排列（横排）
grid-auto-flow : row dense;           // 按照行紧凑排列，自动填充之前的空白（竖排）
```

#### 子项放置（写在子项）
```css
grid-column 是 grid-column-start 和 grid-column-end 的缩写，表示从哪一列开始跨越
grid-column-start : 1；
grid-column-end : 3；
grid-column : 1 / 3;    ->  grid-column : 3 / 1;  ->  grid-column : 1 / span 2;  ->  grid-column : span 2;
```

```css
grid-row 是 grid-row-start 和 grid-row-end 的缩写，表示从哪一行开始跨越
grid-row-start : 1；
grid-row-end : 2；
grid-row : 1 / 2;
```
...
```css
grid-column : 1 / 1 / span 1 / span 2;         // grid-column : 起始行 / 起始列 / 结束行 / 结束列
```
...
文本输的inputmode属性共有8种可能的取值，每种取值对应不同的虚拟键盘类型，旨在优化移动端用户的输入体验。以下是具体分类及说明：

​none <input type="text" inputmode="none" placeholder="点击弹出自定义键盘">
  不显示虚拟键盘，适用于需要自定义键盘的场景（如安全验证码输入）。此时页面需自行实现输入控件。
​text <input type="text" inputmode="text" lang="ja" placeholder="请输入日语文本">
  显示标准文本键盘（默认值），适用于通用文本输入。明确指定此值可覆盖浏览器根据type属性自动选择的键盘类型。
​decimal <input type="text" inputmode="decimal" placeholder="请输入金额（如 99.99）">
  小数输入键盘，包含数字、小数点（.）或千分符（,）。适合金额、计量单位等需要小数位的输入场景。
​numeric <input type="text" inputmode="numeric" placeholder="请输入年龄">
  纯数字键盘（0-9），可能包含减号（-）。适用于年龄、验证码等纯数字输入需求。
​tel <input type="text" inputmode="tel" placeholder="请输入电话号码">
  电话拨号键盘，包含数字、*和#。常用于电话号码输入（尽管推荐优先使用<input type="tel">）。
​search <input type="text" inputmode="search" placeholder="搜索内容">
  优化搜索的键盘，返回键可能显示为“搜索”或“Go”，适合搜索框输入。
​email <input type="text" inputmode="email" placeholder="请输入邮箱地址">
  带@符号的键盘，优化邮件地址输入。常与type="email"配合使用。
​url <input type="text" inputmode="url" placeholder="请输入网址">
  网址专用键盘，突出显示/、.等符号，部分浏览器会添加.com快捷键。适用于URL输入场景。
...
含义为：宽为手机移动设备默认宽度，初始缩放比例为1，最大缩放比例为原始像素大小，不允许用户放大或者缩小;
```
...
##### H5布局策略比较
| 单位 | 相对基准 | 关键优点 | 关键缺点 | H5场景最佳用途 |
| :---: | --- | --- | --- | --- |
| **px** | 固定值 | 精确控制，简单直观 | 无法随用户字体设置或视口缩放，不利于响应式和可访问性 | 边框、阴影等不需要缩放的固定尺寸元素 |
| rem | 根元素 (`<html>`) 的字体大小 | 尊重用户字体偏好，易于全局缩放，利于可访问性 | 无法直接与视口尺寸挂钩，实现自适应布局需JS辅助 | 字体、间距、组件尺寸等需要统一缩放的元素 |
| vw | 视口宽度的1% | 直接与视口宽度关联，实现流畅的响应式布局 | 可能导致在极端尺寸下字体过大或过小；可能忽略用户字体设置，影响可访问性 | 全屏容器、需要精确填充视口宽度的布局元素 |
| 混合 (rem+vw) | 根字体大小和视口宽度 | 结合`rem`的可访问性和`vw`的响应性，通过`calc()`或`clamp()`实现更平滑、有界的缩放 | 增加了计算复杂性，需要更仔细的测试 | 需要在保持基本可读性的同时实现动态缩放的标题或关键UI元素 |
